        ...
        } elseif ($dataStart <= $dateEnd)
        {
            if ($dataStart != 0 || $dateEnd != 0)
            {
                ...
                if($selProvince != '' && strtolower($selProvince) != "all") {                             
                 ...
                 //filter by date
                // $this->view_district_reg_office_model->join('bridge_final_inspection','bri03id = bridge_final_inspection.b_id','left')->where('bridge_final_inspection.bri_completion_fiscal_year',$dataStart)->orWhere('bri_completion_fiscal_year',$dateEnd);
                ...
                if ($intUserType == ENUM_REGIONAL_MANAGER || $intUserType == ENUM_REGIONAL_OPERATOR) {
                  ...
                  $selDist = $selDist->where('bri_completion_fiscal_year',$dataStart)->orWhere('bri_completion_fiscal_year',$dateEnd)->paginate($perPage);
                }
