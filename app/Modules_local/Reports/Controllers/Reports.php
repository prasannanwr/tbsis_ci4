...
class Reports extends BaseController
{
    ...
    public function Gen_Dag_UnderConst_FYWise($ext = '')
    {
        $data = self::$arrDefData;
        $data['view_file'] = __FUNCTION__;
        // 
        $data['arrDistList'] = $this->fiscal_year_model->orderBy('fis01id DESC')->asObject()->findAll();
        $data['blnMM'] = $ext;
        return view('\Modules\Reports\Views\Gen_Dag_UnderConst_FYWise', $data);
    }

    function Gen_Dag_UnderConst_FYWise_report($stat = '')
    {
        $x = dirname(__FILE__) . '/inc/' . __FUNCTION__ . '.php';
        $data = self::$arrDefData;
        $data['view_file'] = __function__;
