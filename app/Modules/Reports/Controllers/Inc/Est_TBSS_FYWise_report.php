<?php
       ...
       $this->load->model('fiscal_year/fiscal_year_model');
        ...
        $data['startdate'] = $this->fiscal_year_model->where('fis01id', $dataStart)->dbGetRecord();
        $data['enddate'] = $this->fiscal_year_model->where('fis01id', $dateEnd)->dbGetRecord();
        if ($Postback == 'Back')
        {
            redirect(site_url());
        } elseif ($dataStart <= $dateEnd)
        {
            if ($dataStart != 0 || $dateEnd != 0)
            {
                ...
                $arrBridgeCostList = $this->view_all_actual_view_model->
                    whereIn('bri07bridge_no', $arrBridgeIdList)->
                    view_bridge_new_estimated_cost();
                    
                foreach ($arrBridgeCostList as $x2)
                {
                    $arrCostList['bri_'.$x2->bri07bridge_no]['id_' . $x2->bri07cmp01id] = $x2;
                }
            ...
            {
                redirect("reports/Est_TBSS_FYWise/".$stat);
            }
        } else
        {
